                                         ...
                                         implements Zend_Mail_Storage_Writable_Interface
{
    ...
    public static function initMaildir($dir)
    {
        if (file_exists($dir)) {
            if (!is_dir($dir)) {
                ...
                throw new Exception('maildir must be a directory if already exists');
            }
        } else {
            if (!mkdir($dir)) {
                /**
                ...
                $dir = dirname($dir);
                if (!file_exists($dir)) {
                    throw new Exception("parent $dir not found");
                } else if (!is_dir($dir)) {
                    throw new Exception("parent $dir not a directory");
                } else {
                    throw new Exception('cannot create maildir');
                }
            }
        }
        ...
        foreach (array('cur', 'tmp', 'new') as $subdir) {
            if (!@mkdir($dir . DIRECTORY_SEPARATOR . $subdir)) {
                ...
                if (!file_exists($dir . DIRECTORY_SEPARATOR . $subdir)) {
                     ...
                     */
                    // require_once 'Zend/Mail/Storage/Exception.php';
                    throw new Exception('could not create subdir ' . $subdir);
                }
            }
        }
    }
    ...
    public function createFolder($name, $parentFolder = null)
    {
        ...
        if (strpos($folder, $this->_delim . $this->_delim) !== false) {
             ...
             */
            // require_once 'Zend/Mail/Storage/Exception.php';
            throw new Exception('invalid name - folder parts may not be empty');
        }
        ...
        if (strpos($folder, DIRECTORY_SEPARATOR) !== false || strpos($folder, '/') !== false
             ...
             */
            // require_once 'Zend/Mail/Storage/Exception.php';
            throw new Exception('invalid name - no directory seprator allowed in folder name');
        }
        ...
        if (!@mkdir($fulldir) || !@mkdir($fulldir . DIRECTORY_SEPARATOR . 'cur')) {
             ...
             */
            // require_once 'Zend/Mail/Storage/Exception.php';
            throw new Exception('error while creating new folder, may be created incompletly');
        }
    ...
    public function removeFolder($name)
    {
        ...
        if ($name == 'INBOX' || $name == DIRECTORY_SEPARATOR || $name == '/') {
             ...
             */
            // require_once 'Zend/Mail/Storage/Exception.php';
            throw new Exception('wont delete INBOX');
        }
        ...
        if ($name == $this->getCurrentFolder()) {
             ...
             */
            // require_once 'Zend/Mail/Storage/Exception.php';
            throw new Exception('wont delete selected folder');
        }
        ...
        foreach (array('tmp', 'new', 'cur', '.') as $subdir) {
            ...
            if (!$dh) {
                 ...
                 */
                // require_once 'Zend/Mail/Storage/Exception.php';
                throw new Exception("error opening $subdir");
            }
            while (($entry = readdir($dh)) !== false) {
                ...
                if (!unlink($dir . DIRECTORY_SEPARATOR . $entry)) {
                     ...
                     */
                    // require_once 'Zend/Mail/Storage/Exception.php';
                    throw new Exception("error cleaning $subdir");
                }
            }
            closedir($dh);
            if ($subdir !== '.') {
                if (!rmdir($dir)) {
                     ...
                     */
                    // require_once 'Zend/Mail/Storage/Exception.php';
                    throw new Exception("error removing $subdir");
                }
            }
        }
        ...
        if (!rmdir($this->_rootdir . '.' . $name)) {
             ...
             */
            // require_once 'Zend/Mail/Storage/Exception.php';
            throw new Exception("error removing maindir");
        }
    ...
    public function renameFolder($oldName, $newName)
    {
        ...
        if (strpos($newName, $oldName . $this->_delim) === 0) {
             ...
             */
            // require_once 'Zend/Mail/Storage/Exception.php';
            throw new Exception('new folder cannot be a child of old folder');
        }
        ...
        if ($oldName == 'INBOX' || $oldName == DIRECTORY_SEPARATOR || $oldName == '/') {
             ...
             */
            // require_once 'Zend/Mail/Storage/Exception.php';
            throw new Exception('wont rename INBOX');
        }
        ...
        if ($oldName == $this->getCurrentFolder()) {
             ...
             */
            // require_once 'Zend/Mail/Storage/Exception.php';
            throw new Exception('wont rename selected folder');
        }
        ...
        foreach (array('tmp', 'new', 'cur') as $subdir) {
            ...
            if (!rename($olddir . $subdir, $newdir . $subdir)) {
                 ...
                 */
                // require_once 'Zend/Mail/Storage/Exception.php';
                throw new Exception('error while moving ' . $subdir);
            }
        }
    ...
    protected function _createTmpFile($folder = 'INBOX')
    {
        ...
        if (!file_exists($tmpdir)) {
            if (!mkdir($tmpdir)) {
                 ...
                 */
                // require_once 'Zend/Mail/Storage/Exception.php';
                throw new Exception('problems creating tmp dir');
            }
        }
        ...
        if (!$fh) {
             ...
             */
            // require_once 'Zend/Mail/Storage/Exception.php';
            throw new Exception("tried $max_tries unique ids for a temp file, but all were taken"
                                                . ' - giving up');
        }
    ...
    protected function _getInfoString(&$flags)
    {
        ...
        if (isset($wanted_flags[Zend_Mail_Storage::FLAG_RECENT])) {
             ...
             */
            // require_once 'Zend/Mail/Storage/Exception.php';
            throw new Exception('recent flag may not be set');
        }
        ...
        if (!empty($wanted_flags)) {
             ...
             */
            // require_once 'Zend/Mail/Storage/Exception.php';
            throw new Exception('unknown flag(s): ' . $wanted_flags);
        }
    ...
    public function appendMessage($message, $folder = null, $flags = null, $recent = false)
    {
        if ($this->_quota && $this->checkQuota()) {
             ...
             */
            // require_once 'Zend/Mail/Storage/Exception.php';
            throw new Exception('storage is over quota!');
        }
        ...
        if (!link($temp_file['filename'], $new_filename)) {
             ...
             */
            // require_once 'Zend/Mail/Storage/Exception.php';
            $exception = new Exception('cannot link message file to final dir');
        }
    ...
    public function copyMessage($id, $folder)
    {
        ...
        if (!copy($old_file, $temp_file['filename'])) {
             ...
             */
            // require_once 'Zend/Mail/Storage/Exception.php';
            $exception = new Exception('cannot copy message file');
    ...
    public function moveMessage($id, $folder) {
        ...
        if ($folder->getGlobalName() == $this->_currentFolder
             ...
             */
            // require_once 'Zend/Mail/Storage/Exception.php';
            throw new Exception('target is current folder');
        }
        ...
        if (!rename($old_file, $new_file)) {
             ...
             */
            // require_once 'Zend/Mail/Storage/Exception.php';
            $exception = new Exception('cannot move message file');
        }
        @unlink($temp_file['filename']);
    ...
    public function removeMessage($id)
    {
        ...
        if (!@unlink($filename)) {
             ...
             */
            // require_once 'Zend/Mail/Storage/Exception.php';
            throw new Exception('cannot remove message');
        }
        unset($this->_files[$id - 1]);
        // remove the gap
        $this->_files = array_values($this->_files);
    ...
    public function getQuota($fromStorage = false) {
        if ($fromStorage) {
            ...
            if (!$fh) {
                 ...
                 */
                // require_once 'Zend/Mail/Storage/Exception.php';
                throw new Exception('cannot open maildirsize');
            }
            $definition = fgets($fh);
            fclose($fh);
    ...
    protected function _calculateMaildirsize() {
        ...
        if (is_array($this->_quota)) {
            ...
            try {
                ...
                throw new Exception('no quota definition found', 0, $e);
            }
        }
